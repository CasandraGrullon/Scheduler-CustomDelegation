...
class ScheduleListController: UIViewController {
  ...
  @IBAction func editButtonPressed(_ sender: UIBarButtonItem) {
    ...
    private func showCreateEventVC(_ event: Event? = nil) { // default parameter
        // use the storyboard to get an instance of the CreateEventController
        guard let createEventVC = storyboard?.instantiateViewController(identifier: "CreateEventController") as? CreateEventController else {
            fatalError("could not downcast to CreateEventController")
        }
        // let createEC = CreateEventController() ---> creates an empty view controller without any outlets, date picker, etc. Will crash!
        
        // to update an event we will add (dependency injection) the selected event
